...
async function loadFromCloud(uid) {
    if (typeof ProgressService === 'undefined') return null;
    try {
        var gifts = await ProgressService.loadCalendarProgress();
        return gifts && Object.keys(gifts).length > 0 ? gifts : null;
    } catch (err) {
...
async function saveToCloud(uid, data) {
    if (typeof ProgressService === 'undefined') return;
    try {
        await ProgressService.saveCalendarProgress(data);
    } catch (err) {
