...
export async function loadGiftsCatalog(options = {}) {
    ...
    if (typeof ConfigService !== 'undefined' && options.preferFirebase) {
        try {
            var gifts = await ConfigService.loadGifts();
            if (gifts && gifts.length) {
                cachedCatalog = normalizeCatalog({ gifts: gifts });
                return cachedCatalog;
            }
        } catch (err) {
            console.warn('ConfigService gifts fallback a JSON:', err);
        }
    }
